...
# Learning how to make a map
```{r}
library(maps)
data("countyMapEnv")
map('county', 'virginia', fill = TRUE, col = palette())
```
...
```{r}
va_map %>% filter(subregion == "charlotte") -> charl
ggplot(data = va_map)+
...
library(viridis)
```
# Saving a Dataframe for the State Outline
```{r}
...
```
Basically, if a county has a percent change of 100%, that means that its share of COVID-19 deaths is double its share of the Virgnia population.
